...
for cp in change_points:
    ...
    if cp < len(df):
        # Get the time value for the change point
        time_point = df['time'].iloc[cp]
        
        fig.add_trace(go.Scatter(
            x=[time_point, time_point],  # Change point time position
            y=[df['value'].min(), df['value'].max()],  # Vertical line from min to max y-value
            mode='lines',
            line=dict(color='red', dash='dash'),  # Dash style for vertical line

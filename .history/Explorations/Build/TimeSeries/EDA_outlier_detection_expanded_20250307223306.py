...
def detect_outliers_arima(df, order=(1, 1, 1)):
    ...
    # outliers_arima.drop["zscore", axis=0, inplace=True]
    # print(outliers_arima.head())
...
def plot_dataset(df, name, window, outliers_iqr, outliers_z, outliers_stl, outliers_arima, change_points):
    ...
    if change_points:
        for cp in change_points:
            # Ensure change point is within the valid range
            if cp < len(df):
                fig.add_trace(go.Scatter(
                    x=[df.index[cp], df.index[cp]], 
                    y=[df['value'].min(), df['value'].max()],
                    mode='lines', name=f"Change Point ({cp})", 
                    line=dict(color='black', dash='dash'), showlegend=True
                ))
            else:
                print(f"Warning: Change point {cp} is out of bounds (max index is {len(df)-1}).")

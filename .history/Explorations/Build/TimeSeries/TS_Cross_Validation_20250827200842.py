...
# train on full series minus last point
train_full = df['Usage'][:-1]
model_full = ExponentialSmoothing(train_full, trend='add', seasonal='add', seasonal_periods=24).fit()
forecast_next = model_full.forecast(1)

plt.figure(figsize=(12,4))
plt.plot(df.index, df['Usage'], label='Full Series')
plt.axvline(df.index[-2], color='gray', linestyle='--', label='Train/Test Split')
plt.scatter(df.index[-1], forecast_next, color='red', label='Forecast Next')
plt.legend()
plt.title("Exponential Smoothing - Forward Chaining Forecast")
plt.show()

...
# ----------------- FORWARD-CHAINING CV (Optimized) -----------------
def forward_chaining_cv_fast(y, initial_train_size=168, seasonal_periods=24, step=6):
    """
    Forward-chaining cross-validation with batch updates for speed.
    - step: number of periods to forecast at once before re-fitting
    """
    n = len(y)
    y_true, y_pred = [], []
    ...
    while i < n:
        ...
        # Fit model once per batch
        model = ExponentialSmoothing(
            train,
            trend='add',
            seasonal='add',
            seasonal_periods=seasonal_periods,
            use_boxcox=False  # specify here, not in .fit()
        ).fit(optimized=True)

        # Forecast next batch
        forecast_end = min(i + step, n)
        forecast = model.forecast(forecast_end - i)

        y_true.extend(y.iloc[i:forecast_end].values)
...
# ----------------- RUN OPTIMIZED CV -----------------
y_true, y_pred = forward_chaining_cv_fast(df['Usage'], initial_train_size=168, seasonal_periods=24, step=6)

# ----------------- VISUALIZE LAST FORECAST -----------------
model_full = ExponentialSmoothing(df['Usage'], trend='add', seasonal='add', seasonal_periods=24).fit()
forecast_5days = model_full.forecast(24*5)

...
def preprocess_data(df, columns_to_explode=None, fill_missing=True, missing_strategy='mean'):
    """Preprocess the dataset with general steps."""
    if fill_missing:
        # Fill missing values for all columns that are numeric
        for column in df.select_dtypes(include=[np.number]).columns:
            df[column] = df[column].fillna(df[column].mean() if missing_strategy == 'mean' else df[column].median())
...
if __name__ == "__main__":
    ...
    df = preprocess_data(df, columns_to_explode=['demand_tags'], fill_missing=True, missing_strategy='mean')

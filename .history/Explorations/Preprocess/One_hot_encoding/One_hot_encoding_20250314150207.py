...
def main():
    ...
    examples_onehotencoder = [
        [0, 1, 3400],  # West Windsor (after one-hot encoding)
        [1, 0, 2800]   # Robbinsville (after one-hot encoding)
    ]
    
    # Using pd.get_dummies
    X_dummies, y_dummies = load_and_preprocess_dummies()
    model_dummies = train_model(X_dummies, y_dummies)
    predictions_dummies = make_predictions(model_dummies, examples_dummies)
    ...
    predictions_labelencoder = make_predictions(model_labelencoder, examples_labelencoder)
    ...
    predictions_onehotencoder = make_predictions(model_onehotencoder, examples_onehotencoder)

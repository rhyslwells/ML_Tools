...
plt.hist(filtered_df_z.height, bins=20, rwidth=0.8, density=True, alpha=0.6, color='b')
rng = np.arange(filtered_df_z.height.min(), df.height.max(), 0.1)
plt.plot(rng, norm.pdf(rng, df.height.mean(), filtered_df_z.height.std()), color='r')
plt.xlabel('Height (inches)')
plt.ylabel('Density')
plt.show()
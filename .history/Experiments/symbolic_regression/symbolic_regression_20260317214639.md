...
Symbolic models allow detection of two distinct drift types when applied to streaming or time-varying data.

**Structural Drift** — Change in expression tree:
- Example: $f_{t-1}(x) = 3x_1 + 0.5x_2$ → $f_t(x) = 3x_1 + 0.5x_2 + \log(x_3)$
- Indicates: new variable influence or new nonlinear relationship

**Parametric Drift** — Structure constant but coefficients shift:
- Example: $f_{t-1}(x) = 3x_1 + 0.5x_2$ → $f_t(x) = 2.6x_1 + 0.7x_2$
- Indicates: same governing relationship, different magnitudes
...
- Maintain Pareto frontier of formulas and track stability across data updates
